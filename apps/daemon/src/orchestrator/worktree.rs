...
impl WorktreeManager {
    ...
    pub fn create_worktree(
        ...
        let task_id_short = &task_id[..task_id.len().min(8)];
        let sanitized_task_name = sanitize_branch_name(task_name);
        let branch_name = format!("{}{}-{}", self.branch_prefix, sanitized_task_name, task_id_short);
    ...
    pub fn cleanup_stale_worktrees(&self, maximum_age_days: u32) -> u32 {
        ...
        let maximum_age_duration =
            std::time::Duration::from_secs(u64::from(maximum_age_days) * 24 * 60 * 60);
        ...
        for worktree_info in &all_worktrees {
            ...
            let worktree_directory_age = match fs::metadata(&worktree_info.worktree_path)
                ...
                .and_then(|modified_time| SystemTime::now().duration_since(modified_time).map_err(|e| std::io::Error::other(e)))
            {
                Ok(age) => age,
                Err(_) => continue,
            };
...
fn sanitize_branch_name(raw_name: &str) -> String {
    ...
    if trimmed_name.len() > 50 {
        trimmed_name[..50].trim_end_matches('-').to_string()
    } else {
        trimmed_name
    }
}
...
fn parse_porcelain_worktree_output(raw_output: &str) -> Vec<WorktreeInfo> {
    ...
    for line in raw_output.lines() {
        if let Some(path_value) = line.strip_prefix("worktree ") {
            ...
            let extracted_branch_name = branch_ref
...
";
        ...
        assert_eq!(
            parsed_results[1].branch_name,
            "kraken/fix-bug-12345678"
        );
        assert_eq!(parsed_results[1].task_id_short, "12345678");

        assert_eq!(
            parsed_results[2].branch_name,
            "kraken/add-feature-87654321"
        );
...
";
    ...
    fn test_create_worktree_in_real_git_repo() {
        ...
        assert_eq!(created_worktree.branch_name, "kraken/fix-login-bug-abcdef12");
        ...
        // Verify the worktree shows up in list
    ...
    fn test_reset_worktree_in_real_git_repo() {
        ...
        // Modify a tracked file and create an untracked file in the worktree
    ...
    fn test_cleanup_stale_worktrees_removes_old_worktrees() {
        ...
        // With max_age_days=0, the just-created worktree should be considered stale
    ...
    fn test_cleanup_stale_worktrees_keeps_recent_worktrees() {
        ...
        // With max_age_days=30, the just-created worktree should NOT be removed
