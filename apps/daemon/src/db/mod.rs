...
fn run_migrations(connection: &Connection) -> SqlResult<()> {
    ...
    // Add `agent` column if it does not already exist (idempotent migration).
    // SQLite does not support IF NOT EXISTS on ALTER TABLE, so we catch the
    // "duplicate column" error and silently continue.
    let agent_column_migration_result = connection
        .execute_batch("ALTER TABLE daemon_tasks ADD COLUMN agent TEXT NOT NULL DEFAULT 'build';");
    match agent_column_migration_result {
        Ok(_) => {}
        Err(ref migration_error) if migration_error.to_string().contains("duplicate column") => {}
        Err(unexpected_migration_error) => {
            return Err(unexpected_migration_error);
        }
    }

    let workdir_column_migration_result =
        connection.execute_batch("ALTER TABLE daemon_tasks ADD COLUMN workdir TEXT;");
    match workdir_column_migration_result {
        Ok(_) => {}
        Err(ref migration_error) if migration_error.to_string().contains("duplicate column") => {}
        Err(unexpected_migration_error) => {
            return Err(unexpected_migration_error);
    ...
    let scheduling_columns: &[&str] = &[
        "ALTER TABLE daemon_tasks ADD COLUMN run_at TEXT;",
        "ALTER TABLE daemon_tasks ADD COLUMN cron_expression TEXT;",
        "ALTER TABLE daemon_tasks ADD COLUMN repeat_interval_seconds INTEGER;",
    ];

    for migration_sql in scheduling_columns {
        match connection.execute_batch(migration_sql) {
            Ok(_) => {}
            Err(ref migration_error)
                if migration_error.to_string().contains("duplicate column") => {}
            Err(unexpected_migration_error) => {
                return Err(unexpected_migration_error);
            }
        }
