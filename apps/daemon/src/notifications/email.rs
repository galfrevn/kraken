...
impl EmailNotificationChannel {
        ...
        match notification_event.event_type {
            ...
            NotificationEventType::DailyDigest => {
                ...
                for detail_key in sorted_detail_keys {
                    ...
                    html_sections.push(format!("<p><b>{}:</b> {}</p>", detail_key, detail_value));
                }
    ...
    pub fn build_resend_api_request_body(&self, notification_event: &NotificationEvent) -> Value {
        let email_subject = Self::format_email_subject_for_event(notification_event);
        let email_html_body = Self::format_email_html_body_for_event(notification_event);
    ...
    async fn send(&self, notification_event: &NotificationEvent) -> Result<(), String> {
        ...
        let http_response = self
            ...
            .map_err(|request_error| {
                format!("failed to send email via Resend API: {}", request_error)
            })?;
...
mod tests {
    ...
    fn resend_api_request_body_has_correct_json_structure() {
        ...
        assert_eq!(request_body["from"], "Kraken <kraken@yourdomain.com>");
