...
impl SystemNotificationChannel {
    pub fn new(channel_name: String, subscribed_event_types: Vec<NotificationEventType>) -> Self {
    ...
    fn format_notification_title_for_event(notification_event: &NotificationEvent) -> String {
        match notification_event.event_type {
            ...
            NotificationEventType::PullRequestCreated => "Kraken: Pull Request Created".to_string(),
    ...
    pub fn build_platform_notification_command(
        ...
        if cfg!(target_os = "macos") {
            ...
            linux_command.arg(notification_title).arg(notification_body);
            Some(linux_command)
...
impl NotificationChannel for SystemNotificationChannel {
    async fn send(&self, notification_event: &NotificationEvent) -> Result<(), String> {
        let notification_title = Self::format_notification_title_for_event(notification_event);
        let notification_body = Self::format_notification_body_for_event(notification_event);

        let mut platform_command =
            Self::build_platform_notification_command(&notification_title, &notification_body)
                .ok_or_else(|| "unsupported platform for desktop notifications".to_string())?;
        ...
        if !command_output.status.success() {
            let stderr_output = String::from_utf8_lossy(&command_output.stderr).to_string();
            return Err(format!(
...
mod tests {
    ...
    fn notification_body_for_task_failed_includes_error() {
        ...
        assert_eq!(body, "fix-login-bug - Compilation failed on line 42");
    }
    ...
    fn notification_body_for_daily_digest_uses_summary() {
        ...
        assert_eq!(body, "Tasks: 5 completed, 1 failed | Total cost: $1.23");
    }
    ...
    fn system_channel_returns_correct_name_and_subscribed_events() {
        ...
        let system_channel =
            SystemNotificationChannel::new("desktop-alerts".to_string(), subscribed_events.clone());
    ...
    fn applescript_escape_handles_quotes_and_backslashes() {
        let escaped = SystemNotificationChannel::escape_for_applescript("He said \"hello\\world\"");
    ...
    fn powershell_escape_handles_single_quotes() {
        let escaped = SystemNotificationChannel::escape_for_powershell("it's a test with 'quotes'");
