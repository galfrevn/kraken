...
impl NotificationEvent {
    pub fn format_as_plain_text(&self) -> String {
        ...
        let mut formatted_output = format!(
            "[{}] {} — {}\nTask: {} ({})\n",
            formatted_timestamp, self.event_type, self.summary, self.task_name, self.task_id,
        );
...
mod tests {
    ...
    fn notification_event_type_display() {
        assert_eq!(
            format!("{}", NotificationEventType::TaskStarted),
            "TaskStarted"
        );
        assert_eq!(
            format!("{}", NotificationEventType::TaskCompleted),
            "TaskCompleted"
        );
        assert_eq!(
            format!("{}", NotificationEventType::TaskFailed),
            "TaskFailed"
        );
        assert_eq!(
            format!("{}", NotificationEventType::PullRequestCreated),
            "PullRequestCreated"
        );
        assert_eq!(
            format!("{}", NotificationEventType::TriggerFired),
            "TriggerFired"
        );
        assert_eq!(
            format!("{}", NotificationEventType::DailyDigest),
            "DailyDigest"
        );
        assert_eq!(
