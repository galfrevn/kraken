...
impl NotificationChannel for GitHubNotificationChannel {
    async fn send(&self, notification_event: &NotificationEvent) -> Result<(), String> {
        ...
        let pull_request_number = match Self::extract_pull_request_number_from_url(pull_request_url)
        {
            Some(number) => number,
            None => return Ok(()),
        };
        ...
        let comment_body_markdown = Self::format_comment_body_as_markdown(notification_event);
...
mod tests {
    ...
    fn extract_pull_request_number_from_standard_url() {
        let pull_request_number = GitHubNotificationChannel::extract_pull_request_number_from_url(
