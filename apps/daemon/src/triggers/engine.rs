...
use super::types::{
    CronTriggerConfig, TriggerEvent, TriggerType, WatcherTriggerConfig, WebhookTriggerConfig,
    render_template,
};
...
const MAX_TASK_NAME_DISPLAY_LENGTH: usize = 120;
const DEFAULT_TRIGGER_PRIORITY: i32 = 5;
const DEFAULT_TRIGGER_AGENT: &str = "build";
...
impl TriggerEngine {
    ...
    async fn handle_cron_event(&self, event: &TriggerEvent) -> Option<String> {
        ...
        for cron_config in cron_configs_guard.iter() {
            ...
            let agent = cron_config
                .agent
                .as_deref()
                .unwrap_or(DEFAULT_TRIGGER_AGENT);
            ...
            return self
                .create_task_from_trigger_with_agent(&rendered_task_description, event, agent)
                .await;
        }
    ...
    async fn handle_slash_command_event(&self, event: &TriggerEvent) -> Option<String> {
        ...
        self.create_task_from_trigger(&task_description, event)
            .await
    }
    ...
    async fn create_task_from_trigger(
        ...
        self.create_task_from_trigger_with_agent(task_description, event, DEFAULT_TRIGGER_AGENT)
            .await
    }

    async fn create_task_from_trigger_with_agent(
        &self,
        task_description: &str,
        event: &TriggerEvent,
        agent: &str,
        ...
        if let Some(ref limiter) = self.rate_limiter {
            let rate_key = format!("trigger:{}", event.source);
            let result = limiter.check(&rate_key);
            if result.is_exceeded() {
                warn!(
                    trigger_source = %event.source,
                    "rate limit exceeded for trigger, skipping task creation"
                );

                let mut details = HashMap::new();
                details.insert("trigger_source".to_string(), event.source.clone());
                details.insert("reason".to_string(), "tasks_per_trigger".to_string());
                ...
                self.event_broadcaster.publish(DaemonEvent {
                    ...
                    summary: format!(
                        "Rate limit exceeded for trigger '{}', task creation skipped",
                        event.source
                    ),
                    details,
        ...
        let task_name = format!(
            "[{}] {}",
            event.trigger_type,
            truncate_string(task_description, MAX_TASK_NAME_DISPLAY_LENGTH)
        );
            ...
            .await
        {
            Ok(created_task) => {
                ...
                trigger_details.insert("trigger_source".to_string(), event.source.clone());
                trigger_details.insert("trigger_type".to_string(), event.trigger_type.to_string());
                ...
                self.event_broadcaster.publish(DaemonEvent {
                    ...
                    summary: format!(
                        "Trigger '{}' fired, created task '{}'",
                        event.source, created_task.name
                    ),
...
mod tests {
    ...
    use crate::triggers::types::{FilterOperator, TriggerFilter, WebhookEventConfig};
    use chrono::Utc;
    use serde_json::json;
    use uuid::Uuid;
    ...
    async fn test_handle_webhook_event_no_matching_config() {
        let task_store = create_test_task_store().await;
        let engine =
            TriggerEngine::new(task_store, vec![], vec![], vec![], EventBroadcaster::new());

        let event =
            create_test_trigger_event(TriggerType::Webhook, "webhook:github:push", json!({}));
    ...
    async fn test_handle_cron_event_matching_config() {
        ...
        assert!(
            created_task
                .description
                .contains("Daily code review for 2026-03-14")
        );
    }
    ...
    async fn test_handle_cron_event_no_matching_config() {
        ...
        let event = create_test_trigger_event(TriggerType::Cron, "cron:unknown-job", json!({}));
    ...
    async fn test_handle_file_change_event_matching_config() {
        ...
        assert!(
            created_task
                .description
                .contains("File changed: src/main.rs")
        );
    }
    ...
    async fn test_handle_slash_command_event() {
        ...
        assert!(
            created_task
                .description
                .contains("Fix the broken CI pipeline")
        );
    }
