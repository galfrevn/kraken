...
impl WebhookServer {
    ...
    pub async fn start(
        ...
        let webhook_bind_address =
            std::env::var("KRAKEN_WEBHOOK_BIND").unwrap_or_else(|_| "127.0.0.1".to_string());
        let listen_address = format!("{webhook_bind_address}:{port}");
        let tcp_listener = TcpListener::bind(&listen_address)
            .await
            .map_err(|bind_error| {
                format!("failed to bind webhook server on {listen_address}: {bind_error}")
            })?;
...
async fn handle_webhook_request(
    ...
    let extracted_event_type =
        extract_event_type_from_headers(&provider_name, &headers, &body_bytes);
...
fn validate_gitlab_token(request_headers: &HeaderMap, webhook_secret: &str) -> Result<(), String> {
...
mod tests {
    ...
    async fn create_test_trigger_engine(
        ...
        let database_path =
            temporary_directory.join(format!("kraken_test_webhook_{}.sqlite", Uuid::new_v4()));
