...
impl TriggerFilter {
    ...
    fn apply_operator(&self, field_value: &Value) -> bool {
        match &self.operator {
            ...
            FilterOperator::StartsWith => value_as_string(field_value).starts_with(&self.value),
            FilterOperator::EndsWith => value_as_string(field_value).ends_with(&self.value),
            FilterOperator::Matches => match Regex::new(&self.value) {
                Ok(compiled_regex) => compiled_regex.is_match(&value_as_string(field_value)),
                Err(_) => false,
            },
        }
    }
    ...
    fn evaluate_contains(&self, field_value: &Value) -> bool {
        if let Some(array_elements) = field_value.as_array() {
            return array_elements
                .iter()
                .any(|element| value_as_string(element) == self.value);
        }
...
pub fn render_template(template: &str, payload: &Value) -> String {
    ...
    while cursor_position < template_length {
            ...
            && template_bytes[cursor_position + 1] == b'{'
        {
            ...
            if let Some(closing_brace_offset) = template[placeholder_content_start..].find("}}") {
                let placeholder_expression = &template
                    [placeholder_content_start..placeholder_content_start + closing_brace_offset];
                let trimmed_expression = placeholder_expression.trim();

                let resolved_text =
                    if let Some(payload_path) = trimmed_expression.strip_prefix("event.") {
            ...
            cursor_position += template[cursor_position..]
                .chars()
                .next()
                .unwrap()
                .len_utf8();
...
mod tests {
    ...
    fn test_render_template_multiple_placeholders() {
        ...
        let rendered = render_template("PR #{{event.number}} in {{event.repo}}", &payload);
    ...
    fn test_render_template_nested_path() {
        ...
        let rendered = render_template("Branch: {{event.pull_request.head.ref}}", &payload);
